...
module MACL
  module Parcer
    ...
    module Regexp
      DTTYPES   = /(a-)?(#{MACL::Parcer.data_types.keys.join(?|)}):(.*)/i
      KEYNVALUE = /(.+):\s*(.+)/i
    ...
    def self.obj2str *objs
      Singulize(objs.map do |obj| String obj end)
    end

    def self.str2bool *strs
      Singulize(strs.map do |str|
        case str.upcase
          when *STRS_TRUE  ; true
    ...
    def self.str2int *strs
      Singulize(strs.map do |str| Integer str end)
    end

    def self.str2flt *strs
      Singulize(strs.map do |str| Float str end)
    end

    def self.str2prate *strs
      Singulize(strs.map do |str| str.to_i/100.0 end)
    end

    def self.str2rate *strs
      Singulize(strs.map do |str| str.to_i/1.0 end)
    end
    ...
    def self.parse_knv_str tag,types=[:nil],has_array=false
      ...
      values.map!{|(n,index)|value2obj(n,types[index]||:nil)}
      return key, values
    end
...
MACL.register('macl/xpan/parcer', '1.4.0')