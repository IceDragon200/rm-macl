#
# rm-macl/lib/rm-macl/xpan/dimension_cells/dimension_cells_base.rb
#   by IceDragon
# DimensionCells are dimension-less container classes, they can be used as
# Vectors, Matrices or complex multi-dimensional Tables.
require 'rm-macl/macl-core'
module MACL
  class DimensionCellsBase

    include Comparable

    class DimensionCellsError < StandardError
    end
    ...
    #   returns an Array of classes which are allowed by the DimensionCells as data
    ...
    ##
    # after_set(int index)
    #   callback function called after data has changed by DimensionCells calls
    def after_set(index)
    ...
    ##
    # bang_do!(other) -> self
    #   abstract function for operating on all @data in the DimensionCells
    def bang_do!(other)
      case other
      when DimensionCells
    ...
    #   abstract function for operating on a select @data in the DimensionCells
    ...
    #   Replaces all the data in the DimensionCells with other
    #   if other is a DimensionCells then it will replace the data using the other's
    #   data
    def replace!(*args)
    ...
    #   see replace!
    #   Operates on a select region of the DimensionCells starting at trg_vec
    def replace_at!(*args)
...
MACL.register('macl/xpan/dimension_cells/base', '1.2.0')