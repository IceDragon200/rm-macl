...
require_relative 'skinj-commands.rb'

$console_out = $stdout 

($imported ||= {})['Skinj'] = 0x14000
# ╒╕ ♥                                                                Skinj ╒╕
# └┴────────────────────────────────────────────────────────────────────────┴┘
class Skinj
  ...
  def self.write_skinj_error_log(skinj)
    ...
    File.open("crashes/#{Time.now.to_f*10**4}.log",?w) do |f|
      ...
      f.puts time_str
      f.puts "Error in file #{""} on line: #{skinj.index}"
      f.puts ex.message
      f.puts ex.backtrace
    end    
  end
